...
The manuscript is organised as follows: first we describe the in situ observations used for the creation of the gridded products; then we present the gridded technique, DIVAnd, that will be applied to the observations; finally we describe the final products (the climatologies) and their validation against existing products. 
...
As stated in Sect.~\ref{sec:insitu}, the datasets are characterised by their strong heterogeneity. To illustrate this feature, Fig.~\ref{fig:phosphatedata} shows the phosphate data points in the 6~regional domains, with the region boundaries defined in Tab.~\ref{tab:regions}. Most of the coastal zones are well sampled, with the exception of the southern part of the Mediterranean Sea east of Tunisia. The North Sea also exhibits obvious differences between the western and eastern parts of the domain, the latter displaying a better data coverage. 
...
The hexbin plot (Fig.~\ref{fig:phosphatedatahexbin}) displays the number of observations in hexagonal cells, it provides another view on the data coverage. The regions with the highest coverage are located in the Baltic Sea and in the southern part of the North Sean while in general lower data densities are found at the edges of the global domain. The Mediterranean sea also exhibits an inhomogeneous data coverage. 

\begin{figure}[t]
\includegraphics[width=8.3cm]{observations_Water_body_phosphate.png}
\caption{Locations of the phosphate concentration profiles.\label{fig:phosphatedatahexbin}}
\end{figure}

The panels of Fig. x also highlights the differences between the variables: for phosphate, silicate and oxygen, most of the domain is covered, whereas for ammonium, dissolved organic nitrogen and, to a lesser extent, chlorophyll, large sub-domains are almost void of measurements. This will impact the quality of the interpolated field (Section …), which depends on the quality of the measurements and their availability.
...
This application was performed in this work, as described in Sect.~\ref{sec:posterioriQC}.
