...
\Author[2]{Karin}{Wesslander}			% 0009-0002-4402-9633
\Author[2]{Lotta}{Fyrberg}				% 0009-0005-0981-2421
\Author[3]{Julie}{Gatti}					% 0000-0001-9327-4366
\Author[3]{Amandine}{Thomas}				% 0000-0002-0441-2446
\Author[4]{Jonas}{Koefoed R{\o}mer}		% 
...
\Author[7]{Ann Kristin}{{\O}strem}		% 0009-0001-2143-8090
...
\Author[10]{Reiner}{Schlitzer}			% 0000-0002-3740-6499
\Author[11]{Dick}{Schaap}				% 0000-0001-6562-068X
...
\affil[2]{Swedish Meteorological and Hydrological Institute (SMHI), Göteborgseskaderns Plats 3, 42671 Västra Frölund, Sweden}		% EDMO: 545
...
\affil[10]{Alfred Wegener Institut, Helmholtz Zentrum für Polar und Meeresforschung, Bremerhaven, Bremen, Germany} 
\affil[11]{Marine Information Service (MARIS), Gildeweg 7 A, 2632BD Nootdorp, Netherlands} 													% EDMO: 634
...
Ocean climatologies provide gridded fields of long-term average ocean variables across various time periods and depth levels, supporting model initialization, data visualization, and insights into the ocean's mean state. While global or regional in scope, most climatologies focus on temperature and salinity, with chemical and biological variables less commonly addressed due to sparser data coverage.

We introduce a set of high-resolution regional climatologies for six eutrophication-related variables (ammonium, chlorophyll-a, dissolved inorganic nitrogen, dissolved oxygen, phosphate, and silicate) covering European seas. These climatologies were generated using the Data-Interpolating Variational Analysis in n dimensions (DIVAnd), a robust multi-dimensional interpolation tool based on variational methods, previously applied to diverse variables and domains. Input data consist of in situ observations, quality-controlled by regional experts following standardised procedures.

The climatologies span multiple spatial scales: a broad domain encompassing all European seas, six regional domains (Arctic Ocean, North-east Atlantic, Baltic Sea, Black Sea, Mediterranean Sea, and North Sea), and four coastal zones (Loire River, Gulf of Riga, Po River, and Danube Delta). Spatial resolution and DIVAnd parameters are tailored to each domain's characteristics. Available as CF-compliant netCDF files, the climatologies can be visualized via the EMODnet Central Portal viewer, enhancing accessibility for research and management applications.
...
The main goal of Chemistry thematic lot of EMODnet is the collection, harmonization and quality control of data gathered from national monitoring efforts and research activities, thus providing relevant information for the assessment of the \textit{Good Environmental Status} \citep[GES,][]{Vinci2017}. The underlying network is constituted by more than 45 National Oceanographic Data Centres (NODCs) from 27 countries. The data collection is centred on three main topics: marine litter \citep{Vinci2018}, marine contaminants (e.g., hydrocarbons, pesticides, heavy metals, antifoulants) and eutrophication \citep{Giorgetti2020}, which is the focus of the present work. 

Throughout the project lifetime, considerable efforts have been undertaken in terms of harmonisation, standardisation and validation of the data and metadata. In practice, the data management is governed by SeaDataNet standards and formats \citep{Schaap2010}, the Infrastructure for Spatial Information in Europe (INSPIRE) rules and the Open Geospatial Consortium (OGC) protocols. 

The aggregated datasets, described in this section and used for the creation of the gridded products (Section~\ref{sec:products}), contain several variables related to eutrophication and acidity of the ocean. These are separated by geographical regions (as described in Section~\ref{sec:domains}).

All the data collections have in common strong heterogeneity, which will be illustrated in the next sections: 
\begin{itemize}
\item spatially: while some regions are densely covered by observations, other regions have barely been sampled (or the existing observations have not made their ways to the NODCs);
...
\end{itemize}
This multiple heterogeneity has to be considered in the gridded fields: the interpolation method has to provide an estimation of the error on the analysis, based on the availability of observations. 
...
The main region of interest encompasses the European Seas (Tab.~\ref{tab:regions} and Fig.~\ref{fig:gebco_bathy_mask_domains3}). The regional domains have been delimited in order to avoid overlap between them:
\begin{itemize}
...
\item The Baltic Sea and the North Sea have their limit located close to the Kattegat (between Denmark and Sweden).
\end{itemize}
...
Within the regional seas, 4~coastal regions have been defined. They are used for the creation of specific products near river mouths (see Section~\ref{sec:clim}) with a finer spatial resolution. Therefore, there is a total of 11~spatial regions where climatologies are created.
...
Each observation is accompanied by a \textit{Common Data Index} \citep[CDI,][]{Schaap2021,Schaap2022}, which provides details about the availability and geographical spreading of marine data sets managed by the SeaDataNet data centres. The index is based on ISO~19115 and ISO~19139 standards, which define metadata for the description of geographic information, and supported by SeaDataNet Common Vocabularies (\url{https://vocab.seadatanet.org/search}). The CDI has been published in the Ocean Data Standards, managed by the Intergovernmental Oceanographic Commission of the UNESCO \citet{Schaap2021}.
...
\begin{enumerate}
\item Metadata completeness: ensures that temporal and spatial information (longitude, latitude, sampling depth, station bottom depth, time) is supplied.
\item Conversion to standard units and harmonisation of parameter names: ensures that a given variable  always is expressed with the same units and  always has the same name (Tab.~\ref{tab:variables}).
\item Quality flagging of data: a value reflecting the quality is assigned to each measurement.
\end{enumerate} 
The last step is described in detail in \citet{Lipizer2023}, where the quality flag setting is described. Particular attention is paid to the range check: each regional sea is divided into sub-domains and the range of acceptable values, for each variable, is set in those sub-domains. For instance, the acceptable chlorophyll concentration values in the Northern Adriatic Sea (strongly influenced by river discharge) are between 0 and 20~\unit{mg/m{^3}}, while this range is narrower in other sub-regions.

\subsection{Spatial distribution}

As stated in Section~\ref{sec:insitu}, the datasets are characterised by their strong heterogeneity. Figure~\ref{fig:phosphatedata} illustrates this feature by displaying the phosphate and the ammonium data points, in the 6~regional domains from Tab.~\ref{tab:regions}. The differences between the spatial distributions of the two variables is obvious, with ammonium mostly sampled in coastal areas, whereas phosphate observations have a more extended spatial coverage.
...
The number of phosphate concentration observations per year and per month are shown in Fig.~\ref{fig:phosphatetime}. The first sub-plot exhibits an overall positive trend from 1960 to 1990 approximately, followed by a more stationary period. The last years have less observations, due to the typical time lag between the data collection and their inclusion into international databases. Another striking feature visible in the histogram is the contributions by the different regions. For instance, data on phosphate concentration in the North Sea only started to appear in the mid 1990's. The Baltic Sea stands out as the largest contributor for this variable, despite its small geographical extension.

The second sub-plot shows the seasonal availability of measurements, with a factor 2 between the number of observations available in the most frequently sampled month (May) and in the least frequently sampled month (December). 
...
\caption{Histogram corresponding to the phosphate concentration observations.\label{fig:histogram_value_Water_body_phosphate}}
\end{figure}


\subsection{File format}

The original data collections are provided as ODV (Ocean Data View) data collections, which is one of the standard formats promoted by SeaDataNet marine data infrastructure \citep{Lowry2023}. The original data can be analyzed and visualized online via webODV at \url{https://emodnet-chemistry.webodv.awi.de/}. In addition to separate ODV collections for the individual regions, this site also provides access to an aggregated collection covering the data of all regions. Such collections are easily read when exported to another format using the ODV software tool \citep{SCHLITZER2002}. In this work, we exported the collections to netCDF files, which in turn are used as input files for the preparation of the climatologies (next Section). Along with the data (coordinates, time, depth and observed values), a set of metadata is assigned to each measurement. Among them we find information about the instrument, the cruise and the institute that performed the measurements. 

\section{The gridding method\label{sec:method}}
%---------------------------------

In this paper, we refer to \textit{gridding} as the process of computing the values of a variable (for instance: sea water temperature) on the nodes of a regular grid, based on observations of that variable at different locations and time periods. The creation of gridded fields using sparse, in situ observations can be performed using different techniques. For simplicity, we will not review all of them but will concentrate on the distinction between two groups of techniques:
\begin{enumerate}
...
\item The approximation techniques, also called analysis, where the field does not have to contain all the data points. Such a category of technique is often preferred when it comes to processing environmental data.
\end{enumerate}
...
The Data-Interpolating Variational Analysis in n dimensions method \citep[DIVAnd][]{BARTH2014} is designed to grid arbitrarily located observations in two or more dimensions, onto a curvilinear grid. Typically the dimensions are the longitude, the latitude, the depth and the time of the observations (hence 4D). DIVAnd is an evolution of the DIVA tool \citep{TROUPIN2012,BECKERS2014}, which was limited to 2-dimensional interpolations (hence climatologies were built by stacking 2-dimensional layers at different depths and for different periods). 
...
The source code is written in Julia \citep{Bezanson2017}, a fast, dynamically-typed programming language, designed to ensure high-performance computation. Julia ensures that high-resolution analysis using several millions of data points can be run in reasonable times, i.e. maximum a few days for the largest domain. This is usually necessary to create a regional climatology, it is necessary to repeat the analysis several times in order to adapt the parameters and possibly remove suspect observations.

DIVAnd has already been applied applied on a wide range of ocean variables such as temperature, salinity \citep{COATANOAN2021}, sea surface height \citep{DOGLIONI2023}, velocity \citep{TROUPIN2022}, dissolved inorganic nutrients \citep{BELGACEM2021} and oxygen concentration \citep{CLIMATO2023}. In this work, the version v2.7.12 of the code \citep{BARTH2024DIVAnd} is used.
...
The GEBCO bathymetry \citep[2021 grid,][]{GEBCO2021}, with a resolution reduced by a factor 4 with respect to the original, has been used for the creation of the land-sea mask
needed by DIVAnd (Fig.~\ref{fig:gebco_bathy_mask_domains3}). The role of the land-sea mask is to delimit the region where the interpolation is performed, since the cost function (see previous Section) is only computed over cells corresponding to the sea.
...
\item The \textit{Coastal Area} products: they are computed over small areas near river mouth, and at seasonal scale. The spatial resolution is increased in accordance with the Sea Region climatologies.
\end{enumerate}

\subsection{File format}

The output of the analysis is a set of netCDF files \citep{Rew1990,Brown1993,Rew2006} storing the gridded fields generated by the analysis, making use of the NCDatasets.jl library \citep{Barth2024}. The file sizes depend on the type of product (as defined in Section~\ref{sec:products}) and the region of interest. The largest files of approximately 10 ~\unit{GB} originate from the North-East Atlantic region.

In order to reduce the file sizes (optional), advanced compression techniques \citep{Silver2017,Zender2016} can be applied through the use of the NCO tool \citep[netCDF operators,][]{Zender2008}. A file size reduction by a factor of 3-10 can be expected depending on the algorithm and the original file.

\subsection{Gridded variables}

The main variable stores the 4-dimension gridded field (e.g., longitude, latitude, time and depth) for each of the 6 selected variables. In addition, a relative error field stating the quality of the results typically exhibits larger values in regions where no observations were collected, while the values are lowest where the coverage is most dense. The term "relative" means that the error field is scaled by the background variance. The reason why the relative error (instead of the absolute error) is provided arises from the difficulty to compute the background variance.  

The error field is used to mask out the gridded field in regions where the error is considered to be too high. The decision to compute those masked fields comes from discussions among the EMODnet-Chemistry experts, who recommended displaying only the gridded fields where (enough) observations are available. In practice, gridded fields masked using relative error thresholds of 30\% and 50\% are calculated. The field masked with the 30\% error threshold is displayed as the principal layer in the Web Map Service (WMS). The motivation is to prevent users from interpreting the results where no background information exists. 
...
As all EMODnet products, the climatologies have to be available through the EMODnet product catalog \citep[][https://emodnet.ec.europa.eu/en]{BEJA2024}
...
The climatologies can be exploited in various ways and by a diverse user group. The following examples are not meant to be exhaustive, they merely offer a few relevant applications.
...
Combining information about the nutrients and the oxygen in a given region allows you to evaluate the ecosystem health and examine its role in terms of primary production or carbon sequestration.

Concretely, since 2009, EMODnet Chemistry has interacted with the stakeholders involved in the Regional Seas Conventions (OSPAR, HELCOM, UNEP/MAP, BSC \ldots) and the Marine Strategy Framework Directive (MSFD). A Board of experts on the MSFD, formed in 2017, aims to advise the consortium on the products and services: parameters, spatial and temporal resolutions of the data products. Consequently, the data collections and the climatologies will contribute to the MSFD Descriptors D5 (eutrophication), amongst others.

\conclusions  
This paper describes the creation of climatologies for eutrophication variables (ammonium, chlorophyll-a, dissolved inorganic nitrogen, dissolved oxygen, phosphate and silicate) in 11 regional domains. Three groups of regions are considered: All European Seas, Regional Seas and Coastal Zones. The climatologies are based on in situ measurements which have undergone extensive quality control. Those datasets are then fed as in input to the DIVAnd software tool. 

The products consist of netCDF files storing the gridded fields for the specified time periods and depth levels. They can be downloaded and reused for many purposes like visualisation, improved quality control, validation of new measurements, \ldots. 

Possible improvements depend on the increased availability of data, coming from acquisition of new data, but also from initiatives such as EMODnet Data Ingestion \citep{IONA2024}. This initiative strives to reach out, motivate and support potential data providers, in order to make sure that the data they collected can make their way to centralised portals with standardised and complete metadata, hence strengthening the "Collect once, use many times", as mentioned in the introduction. 

The interpolation technique can also be improved, by allowing for instance the interpolation along isopycnals, rather than at fixed depths. This would reduce the mixing effects and allow for a better representation of the water masses. Most of the recent developments of DIVAnd are driven by issues or suggestions raised by the user community, and future developments will continue that principle. Finally, a fundamental aspect of the climatology preparation is to make sure that any external user can take an existing piece of code (Julia script or Jupyter notebook), modify it and tune it according to the specificities of their region and their datasets. 
...
\authorcontribution{CT prepared the manuscript. ME coordinated the management of metadata (catalog and DOI). AB and CT: prepared the pan-European sea products; AB and JMB improved the DIVAnd code. Regional contributions: SI and MT: Mediterranean Sea; K.W.: Baltic Sea; MML and JKR: North Sea; LB and GS: Black Sea; AKO: Arctic Sea; JG and NP: North Atlantic Ocean. ML: data validation protocols.} 

\competinginterests{The authors declare that there is no competing interest.}
\disclaimer{The data coverage obtained with the in situ measurements is heterogeneous, leading to high uncertainties in the gridded field. The information contained in the gridded fields is believed to be trustworthy. However its accuracy and completeness cannot be guaranteed.  Whilst every effort has been made to ensure its reliability within the limits of present knowledge, no responsibility can be accepted by those involved in its compilation or publication for any consequential loss or damage arising from its use.
} 
