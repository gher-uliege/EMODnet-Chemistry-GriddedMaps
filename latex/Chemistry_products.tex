...
The manuscript is organised as follows: first we describe the in situ observations used for the creation of the gridded products; then we present the gridded technique, DIVAnd, that will be applied to the observations; finally we describe the final products (the climatologies) and their validation. 
...
Each observation comes with a \textit{Common Data Index} \citep[CDI,][]{Schaap2022}, which provides details about the availability and geographical spreading of marine data sets managed by the SeaDataNet data centres. 
The index is based upon ISO 19115 and ISO 19139 standards, which define metadata for the description of geographic information, and supported by SeaDataNet Common Vocabularies (\url{https://vocab.seadatanet.org/search}). Thanks to the CDI records, it is possible to keep track of the information regarding the data originators (\url{https://cdi.seadatanet.org/search}), ensuring that their effort is correctly acknowledged in the product metadata. The CDI has been published in the Ocean Data Standards, managed by the Intergovernmental Oceanographic Commission of the UNESCO \citet{Schaap2019}.
